...
function checkMusicGenerated(){
	...
	if (done) {
		...
		music.src = URL.createObjectURL(new Blob([wave], {type: "audio/wav"}));
		music.loop = true;
		musicGenerated = true;
	}
}
...
var zzfx_v=.5,zzfx_x=new AudioContext,zzfx=(e,f,a,b=1,d=.1,g=0,h=0,k=0,l=0,pan=0)=>{let S=44100,P=Math.PI;a*=2*P/S;a*=1+f*(2*Math.random()-1);g*=1E3*P/(S**2);b=0<b?S*(10<b?10:b)|0:1;d*=b|0;k*=2*P/S;l*=P;f=[];for(var m=0,n=0,c=0;c<b;++c)f[c]=e*zzfx_v*Math.cos(m*a*Math.cos(n*k+l))*(c<d?c/d:1-(c-d)/(b-d)),m+=1+h*(2*Math.random()-1),n+=1+h*(2*Math.random()-1),a+=g;e=zzfx_x.createBuffer(1,b,S);a=zzfx_x.createBufferSource();e.getChannelData(0).set(f);a.buffer=e;
	var panN=zzfx_x.createStereoPanner();
	panN.pan.value = pan;
	panN.connect(zzfx_x.destination);
a.connect(panN);a.start();return a}

function playSFX(sfx, loc){
	if (data.sound){
		var x = (loc-canvas.width/2)/(canvas.width);
		data.sfxDefs[sfx](x);
	}
}